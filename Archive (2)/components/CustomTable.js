...
function CustomTable({ shouldMakeButton, getData, getFilteredRows, fields, containsCollapsable, getRowValues, onSave, onEdit, onAdd, onDelete, generateCollapsable, geteditableRow }) {
    ...
    const handleInputChange = (e, index, a=null) => {
        const list = [...rowData];
        if (e.target){
    ...
    const makeSaveOrEditButton = () => {
        if (currentlyEditing || currentlyAdding) {
            return makeButton(<DoneIcon />, "SAVE", () => {
                 onSave(rowData, currentlyAdding, currentlyEditing);
                 if (currentlyAdding){
    ...
    const makedeleteButton = (i) => {
        ...
        <Button className="mr10" onClick={handleConfirm} >
            {(currentlyEditing || currentlyAdding)? <ClearIcon /> : <DeleteOutlineIcon />}
        </Button>
        {showConfirm && (
            <div>
                <Dialog
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
                    ...
                    <div className="TableButtons">
                        {makeButton(<AddBoxIcon />, "ADD", () => { setcurrentlyAdding(true); onAdd(rowData, setRowData); }, shouldMakeButton.Add)}
                        ...
                        <TableBody>
                            {getFilteredRows(rowData, searchQuery).map((row, i) => {
                                return <React.Fragment>
                                    {(currentlyEditing || currentlyAdding) ? geteditableRow(row, i, handleCheckChange, handleInputChange, makedeleteButton) : <ViewRowNew columns={getRowValues(row)} isCollapsable={containsCollapsable} generateCollapsable={generateCollapsable } deleteButton={makedeleteButton} index={i} />}
